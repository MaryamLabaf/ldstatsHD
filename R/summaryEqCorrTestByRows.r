...
summary.eqCorTestByRows <- function(object, ...){
  x <- object
  ...
  if(any(x$testStatistic == "max"))
    cat(gettextf("number of significant rows for Tm: %s at %s conf.level, expected %s", sum(x$ciMax[,1]>0), round(x$conf.level,3), round(dim(x$ciAS)[1]*(1-x$conf.level),2)),"\n\n")  
