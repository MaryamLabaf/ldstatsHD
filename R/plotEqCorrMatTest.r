plot.eqCorTestByRows <- function(x, mains = c("AS CI", "max CI", "exc CI"), xlabs = c("","",""), ylabs = c("","", ""), pch = "-", ownCols =TRUE, ...){
  if (!inherits(x, "eqCorrMatTest")) stop("use only with \"eqCorrMatTest\" object")
