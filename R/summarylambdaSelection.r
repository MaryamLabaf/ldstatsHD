...
summary.lambdaSelection <- function(object, ...){
  x <- object
  ...
  if (!inherits(x, "lambdaSelection"))
   ...
   if(length(x$criterion)==2)
   {
    ...
    cat(gettextf("optimal lambda = %s, \t Sparsity graph structure = %s", round(x$opt.lambda[1],4), round(1-SP1,4)), "\n\n")
    cat(gettextf("optimal lambda = %s, \t Sparsity graph structure = %s", round(x$opt.lambda[2],4), round(1-SP2,4)), "\n\n")
   }
   else{ 
